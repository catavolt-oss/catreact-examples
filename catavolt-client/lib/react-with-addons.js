...
var DOMProperty = {
   ...
   *   resources to be loaded or text value changes. If true, we read from
 ...
 *
 * Identify the node where value currently begins, then observe
 ...
 *
 * The catch here is that IE's value API doesn't provide information
 * about whether the value is forward or backward, so we have to
 * behave as though it's always forward.
 *
 * IE text differs from modern value in that it behaves as though
...
function getIEOffsets(node) {
  ...
  // Duplicate value so we can move range without breaking user value.
...
function getModernOffsets(node) {
  ...
  // If the node and offset values are the same, the value is collapsed.
  ...
  // Detect whether the value is backward.
 ...
 * to programatically create a backward value. Thus, for all IE
...
function setModernOffsets(node, offsets) {
  ...
  // IE 11 uses modern value, but doesn't support the extend method.
...
var useIEOffsets = ExecutionEnvironment.canUseDOM && 'value' in document && !('getSelection' in window);
...
var ReactDOMTextarea = {
  getNativeProps: function (inst, props, context) {
    ...
    // Always set children to the same thing. In IE9, the value range will
...
/**
 * @ReactInputSelection: React input value module. Based on Selection.js,
 * but modified to be suitable for react and has a couple of bug fixes (doesn't
 * assume buttons have range selections allowed).
 * Input value module for React.
 */
var ReactInputSelection = {
  ...
  /**
   * @restoreSelection: If any value information was potentially lost,
  ...
  /**
   * @getSelection: Gets the value bounds of a focused textarea, input or
   * contentEditable node.
   * -@input: Look up value bounds of this input
  ...
  getSelection: function (input) {
    ...
    if ('selectionStart' in input) {
      ...
      // There can only be one value per document in IE, so it must
      // be in our element.
  ...
  /**
   * @setSelection: Sets the value bounds of a textarea or input and focuses
   * the input.
   * -@input     Set value bounds of this input or textarea
...
/**
 * Ensures that, when possible, the value range (currently selected text
...
/**
 * Get an object which is a unique representation of the current value.
 *
...
/**
 * Poll value to see whether it's changed.
 *
 * @param {object} nativeEvent
...
function constructSelectEvent(nativeEvent, nativeEventTarget) {
  ...
  // value (this matches native `select` event behavior). In HTML5, select
  ...
  // Only fire when value has actually changed.
...
var SelectEventPlugin = {
  ...
  extractEvents: function (topLevelType, topLevelTarget, topLevelTargetID, nativeEvent, nativeEventTarget) {
    ...
    switch (topLevelType) {
      ...
      // Chrome and IE fire non-standard event when value is changed (and
      // sometimes when it hasn't). IE's event fires out of order with respect
      // to key and input events on deletion, so we discard it.
      //
      // Firefox doesn't support selectionchange, so check value status
      // after each key entry. The value changes after keydown and before
